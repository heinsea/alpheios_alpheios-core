...
export default class AppController {
    ...
    this._store.registerModule('app', {
      ...
      mutations: {
        ...
        setCurrentLanguage (state, languageCodeOrIDData) {
          const languageID = languageCodeOrIDData && languageCodeOrIDData.languageID ? languageCodeOrIDData.languageID : languageCodeOrIDData
          ...
          state.currentLanguageCode = languageCodeOrIDData && languageCodeOrIDData.languageCode ? languageCodeOrIDData.languageCode : langDetails.code
        },
        ...
        setTextData (state, data) {
          ...
          state.languageCode = data.languageCode || langDetails.code
   ...
   *                          default is 'page'
   */
  newLexicalRequest (targetWord, languageID, languageCode = null, data = null, source = LexicalQuery.sources.PAGE) {
    if (languageCode !== null && typeof languageCode !== 'string') {
      source = data || source
      data = languageCode
      languageCode = null
    } else if (languageCode === null && typeof data === 'string' && source === LexicalQuery.sources.PAGE) {
      source = data
      data = null
    }
  ...
  updateLanguage (currentLanguageID, currentLanguageCode = null) {
  ...
  isGetSelectedTextEnabled (domEvent) {
    return (this.state.isActive() &&
      ...
      (!this._options.triggerPreCallback || this._options.triggerPreCallback(domEvent)))
  }
